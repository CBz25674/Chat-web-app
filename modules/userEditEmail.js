...
module.exports = (req, res) => {
    ...
    conn.query("SELECT `password` FROM `account` WHERE `username`=?", [req.session.user.username], (error, result, field) => {
        bcrypt.compare(req.body.password, result[0].password, (correct) => {
            ...
            conn.query(sqlQuery, [req.body.email], async (err, result, field) => {
                if (err) throw err;
